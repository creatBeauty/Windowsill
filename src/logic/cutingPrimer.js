document.addEventListener('DOMContentLoaded', function () {
  ...
  Rect.prototype.cutVertical = function (cutPosX, cutWidth) {
    ...
    return [
      ...
      new Rect(
        this.x + cutPosX + cutWidth,
        this.y,
        this.w - cutPosX - cutWidth,
        this.h
      ),
    ];
  };
  ...
  Rect.prototype.cutHorizontal = function (cutPosY, cutWidth) {
    ...
    return [
      ...
      new Rect(
        this.x,
        this.y + cutPosY + cutWidth,
        this.w,
        this.h - cutPosY - cutWidth
      ),
    ];
  };
  ...
  Rect.prototype.fitAny = function (tile) {
    return (
      (this.w >= tile.width && this.h >= tile.height) ||
      (this.w >= tile.height && this.h >= tile.width)
    );
  };
  ...
  function main() {
    // var basePlate = new Rect(0, 0, 3600, 760);
    // var tiles = sortTiles([
    //   new Tile('T0', 1600, 200),
    //   new Tile('T1', 500, 300),
    //   new Tile('T2', 600, 300),
    //   new Tile('T4', 600, 100),
    //   new Tile('T3', 80, 40),
    // ]);
    var basePlate = new Rect(0, 0, 760, 3600);

    // Создаем массив плиток из inputData
    var tiles = [];

    inputData.forEach((item) => {
      item.sizes.forEach((size) => {
        const [id, dimensions] = size.split(' '); // Разделяем идентификатор и размеры
        const [width, height] = dimensions.split('X').map(Number); // Получаем ширину и высоту
    ...
    var maxBottomY = getMaxBottomY(root);
    console.log(
      'Расстояние от начала раскроя до самой нижней точки размещенных деталей:',
      maxBottomY
    );
  }
  ...
  function getMaxBottomY(node) {
    if (!node) return 0; // Если узел пустой, возвращаем 0

    let maxBottomY = 0; // Инициализируем максимальную нижнюю координату

    // Проверяем, есть ли плитка в текущем узле
    if (node.tileId) {
      maxBottomY = node.rect.y + node.rect.h; // Нижняя координата текущего узла
    }

    // Рекурсивно проверяем левого и правого потомков
    if (node.left) {
      maxBottomY = Math.max(maxBottomY, getMaxBottomY(node.left));
    }
    if (node.right) {
      maxBottomY = Math.max(maxBottomY, getMaxBottomY(node.right));
    }

    return maxBottomY; // Возвращаем максимальную нижнюю координату
  }
